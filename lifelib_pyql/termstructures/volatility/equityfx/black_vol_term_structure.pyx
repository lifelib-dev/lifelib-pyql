...
from lifelib_pyql.time._date cimport Date as _Date
from lifelib_pyql.time.daycounter cimport DayCounter
from lifelib_pyql.time._daycounter cimport DayCounter as _DayCounter
from lifelib_pyql.time.calendar cimport Calendar
from lifelib_pyql.time._calendar cimport Calendar as _Calendar
from lifelib_pyql.time.businessdayconvention cimport BusinessDayConvention, Following
from lifelib_pyql.termstructures.vol_term_structure import VolatilityTermStructure
...
cdef class BlackVolTermStructure(VolatilityTermStructure):
    ...
    def blackForwardVol(self, time_1, time_2, Real strike, bool extrapolate=False):
        ...
        if isinstance(time_1, float) and isinstance(time_2, float):
            ...
            return self.get_bvts().blackForwardVol(
                (<Date>time_1)._thisptr,
                (<Date>time_2)._thisptr,
                strike,
                extrapolate
            )
        else:
